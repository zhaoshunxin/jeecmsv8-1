...
<#elseif type=="6">
	<#if name='attr_province' || name='attr_department'>
		<@p.select colspan=colspan width=width label=label  name=name value=value help=help list=list listKey="value" listValue="name" helpPosition=helpPosition required=required class=(required=="true")?string("required","")/>
	<#else>
