...
public abstract class HibernateSimpleDao {
	...
	@SuppressWarnings("unchecked")
	protected Pagination findBigData_flm2(SQLQuery query, int pageNo, int pageSize, SQLQuery query_count) {
		int totalCount = Integer.parseInt(query_count.uniqueResult().toString());
		//query.list().size();此语句执行速度特别慢，不能用此语句获取查询结果总数量
		Pagination p = new Pagination(pageNo, pageSize, totalCount);
		if (totalCount < 1) {
			p.setList(new ArrayList());
			return p;
		...
		list = query.list();
		p.setList(list);
		//System.out.println("######################" + c.getCts().toArray().toString());
		return p;
	}
