...
//			List<Channel> topList = channelMng.getTopListByRigth(userId,siteId, true);
		...
		List<CmsDictionary> dic_origin = dictionaryMng.getList("来源");
		List<CmsDictionary> dic_province = dictionaryMng.getList("省市");
		List<CmsDictionary> dic_department = dictionaryMng.getList("司局");
		model.addAttribute("dic_origin", dic_origin);
 		model.addAttribute("dic_province", dic_province);
 		model.addAttribute("dic_department", dic_department);
 		System.out.println("size: " + dic_origin.size());
		return "content/add";
	}
...
//			rights = user.getChannels(siteId);
	...
	public String save(Content bean, ContentExt ext, ContentTxt txt,ContentDoc doc,
		...
		//kjb 如果为某一特定模型（政府信息公开模型），执行sql向数据库中插入索引号
		if(modelId == 12){
			Map<String, String> map = bean.getAttr();
			System.out.println("--------------------" + map.get("department"));
			Date date = new Date();
			DateFormat df = new SimpleDateFormat("yyyy");
			manager.generateContentIndex(bean.getId(), "indexNum", "306-" + map.get("department") + "-" + df.format(date));
		}
		/////////////////
	...
	public String update(Integer queryShare,String queryStatus, Integer queryTypeId,
		...
		///////////////////////////////kjb
		Map<String, String> map = attr;
		if(map.get("department") != null){
		Content old_c = manager.findById(bean.getId());
		Map<String, String> old_m = old_c.getAttr();
		if(!old_m.get("department").toString().equals(map.get("department"))){
		
		//boolean flag = manager.updateContentIndex(bean.getId(), "indexNum", "306-" + map.get("department") + "-" + df.format(date));
		}
		}
		/////////////////////////
	...
	public String priority(Integer[] wids, Byte[] topLevel,Integer queryShare,
		...
		for(int i=0;i<wids.length;i++){
			...
			manager.update(c);
			ContentChannel cc = null;
			cc = content_channel_manager.findById(wids[i],cid);
			if(cc == null){
				cc = content_channel_manager.findById(wids[i], c.getChannel().getId());
			}
			cc.setKjb_sort(topLevel[i]);
			content_channel_manager.update(cc);
		}
	...
	public String uploadAttachment(
		...
		try {
			...
			} else if (site.getSyncPageFtp() != null) {
				Ftp ftp = site.getSyncPageFtp();
...
//		CmsSite site = CmsUtils.getSite(request);
	...
	@Autowired
	private ContentChannelMng content_channel_manager;
}