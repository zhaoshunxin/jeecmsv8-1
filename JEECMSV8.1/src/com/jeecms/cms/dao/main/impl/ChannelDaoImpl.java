...
public class ChannelDaoImpl extends HibernateBaseDao<Channel, Integer>
	...
	private Finder getChildFinder_orderBy(Integer parentId, boolean hasContentOnly,
			boolean displayOnly, boolean cacheable, String orderBy) {
		Finder f = Finder.create("from Channel bean");
		f.append(" where bean.parent.id=:parentId");
		f.setParam("parentId", parentId);
		if (hasContentOnly) {
		...
		switch(orderBy){
			case "1": f.append(" order by bean.id asc");break;
			case "2": f.append(" order by bean.id desc");break;
			default: f.append(" order by bean.priority asc,bean.id asc"); 
		}
	...
	@Override
	public List<Channel> getChildList_orderBy(Integer parentId, boolean hasContentOnly, boolean displayOnly,
			boolean cacheable, String orderBy) {
		Finder f = getChildFinder_orderBy(parentId, hasContentOnly, displayOnly,
				cacheable, orderBy);
		return find(f);
	}
}